const express = require('express');
const bodyParser = require('body-parser')
const app = express();

const pumpkingData = require('./data/pumpkin.json');
const broccoliData = require('./data/broccoli.json');
const nodejsData = require('./data/nodejs.json');
app.use(bodyParser.json());

app.get('/', (req, res) => {
    const randomIndex = Math.floor(Math.random() * pumpkingData.Categories['Pumpkin Drinks'].length);
    const randomDrinkRecipe = pumpkingData.Categories['Pumpkin Drinks'][randomIndex];
    
    res.status(200).json({randomDrinkRecipe});
});

app.get('/pumpkin', (req, res) => {
    res.status(200).json({message: 'Got pumkin?'});
});

app.post('/rke143', (req, res) => {
    
    if(req.body.username !== 'rke' || req.body.password !== '143'){
        res.status(200).json({message: 'Invalid credentials!'}); 
        
    }else{
        res.status(200).json({nodejsData})    
    }
});

app.post('/pumpkin', (req, res) => {
    console.log(req.body);

    if(req.body.username !== 'admin' || req.body.password !== '1234'){
        res.status(200).json({broccoliData})
    }else{
        res.status(200).json({pumpkingData})       
    }

    res.status(200).json({message: 'Welcome'});
});
app.listen(3000, () => {
    console.log("Server is running on port 3000.");
});